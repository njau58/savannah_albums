...
export async function fetchUserAlbums(userId: string): Promise<AlbumProps[]> {
	const response = await fetch(
		`${process.env.NEXTAUTH_URL}/api/users/albums/${userId}`
	)
...
export const fetchAlbumPhotos = async (albumId: string) => {
	const response = await fetch(
		`${process.env.NEXTAUTH_URL}/api/users/albums/photos/${albumId}`
	)
...
export const fetchPhotoById = async (photo_id: string) => {
	const response = await fetch(
		`${process.env.NEXTAUTH_URL}/api/users/albums/photo/${photo_id}`
	)
	if (!response.ok) {
...
export async function editPhotoTitle(photo_id: string, newTitle: string) {
	const response = await fetch(
		`${process.env.NEXTAUTH_URL}/api/users/albums/photo/update/${photo_id}`,
		{
			method: "PATCH",
			headers: {
